...
def upload(database):
    ...
    for g_i in range(DATABASE_CONF[database]["number_group"]):
        ...
        for im_i in range(DATABASE_CONF[database]["number_img"]):
            print(DATABASE_CONF["ORL"]["img_path"].format(g=g_i + 1, im=im_i + 1))
            img = database_data[g_i * 10 + im_i] * 255
            cv2.imwrite(
                DATABASE_CONF["ORL"]["img_path"].format(g=g_i + 1, im=im_i + 1), img
            )
...
def load(database):
    ...
    for g_i in range(DATABASE_CONF[database]["number_group"]):
        ...
        for im_i in range(DATABASE_CONF[database]["number_img"]):
            img = cv2.imread(
                DATABASE_CONF["ORL"]["img_path"].format(g=g_i + 1, im=im_i + 1), -1
            )
